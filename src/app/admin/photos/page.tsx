...
import { getDogs, adminApplyPhotoMap } from "@/lib/store";
...
export default function AdminPhotosPage() {
  ...
  // Estadísticas de ejemplares actuales
  const [stats, setStats] = useState({ total: 0, withPhoto: 0 });

  useEffect(() => {
    const dogs = getDogs();
    setStats({ total: dogs.length, withPhoto: dogs.filter(d => d.photo).length });
  }, [result]);
  ...
  async function loadMap() {
    ...
    try {
      ...
      // Construir preview cruzando con los perros del registro
      const dogs = getDogs();
      const bycert = new Map<string, Dog>(dogs.filter(d => d.certificateId).map(d => [d.certificateId, d]));
      ...
      // Ordenar: primero los que tienen match en el registro
  ...
  function applyPhotos() {
    if (!photoMap) return;
    setApplying(true);
    // setTimeout para no bloquear el hilo de render
    setTimeout(() => {
      try {
        const r = adminApplyPhotoMap(photoMap, overwrite);
        setResult(r);
      } finally {
        setApplying(false);
      }
  ...
  const alreadyHavePhoto = preview.filter(e => e.dog?.photo).length;
    ...
    <AdminLayout>
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Vincula automáticamente las fotos del sitio original (abciregistro.com) a los ejemplares importados.
        </p>
      </div>
        ...
        <Card className="max-w-2xl">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Se extrajo de los archivos WXR de WordPress un mapa que vincula cada número de registro ABCI
                con la URL de la foto en el servidor original (<span className="font-mono text-xs">abciregistro.com</span>).
                Al aplicarlo, las fotos se cargarán desde ese servidor sin ocupar espacio en este sitio.
              </p>
      ...
      {photoMap && !result && (
        <div className="space-y-6 max-w-3xl">
          ...
          <Card>
            ...
            {!showPreview && (
              <p className="text-sm text-muted-foreground">Las fotos se cargan desde <span className="font-mono text-xs">abciregistro.com/wp-content/uploads/</span>.</p>
            )}
          </Card>
